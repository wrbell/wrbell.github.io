...
test.describe("Visual regression", () => {
  test("hero section — dark mode @chromium-desktop", async ({ page, browserName }) => {
    test.skip(browserName !== "chromium", "visual baselines are chromium-desktop only");
  ...
  test("chrono view with Experience filter — dark mode @chromium-desktop", async ({ page, browserName }) => {
    test.skip(browserName !== "chromium", "visual baselines are chromium-desktop only");
    await page.setViewportSize({ width: 1280, height: 800 });
    await page.goto("/");
    await page.waitForLoadState("networkidle");
  ...
  test("full page — dark mode @iphone-safari", async ({ page, browserName }, testInfo) => {
  ...
  test("404 page — dark mode @chromium-desktop", async ({ page, browserName }) => {
    test.skip(browserName !== "chromium", "visual baselines are chromium-desktop only");
    await page.setViewportSize({ width: 1280, height: 800 });
